...
if __name__=="__main__":
    ...
    print(rag_query(collection, "What is ChromaDB used for?", stream=False))

    # Test 2 (related but not in docs): "How does Pinecone compare to ChromaDB?" → said it didn't have enough info
    print(f"\nTest 2: Related but not in documents")
    print(rag_query(collection, "How does Pinecone compare to ChromaDB?", stream=False))
    # Test 3 (out of scope): "What is the best pizza recipe?" → correctly refused
    print(f"\nTest 3: Out-of-scope question")
    print(rag_query(collection, "What is the best pizza recipe?", stream=False))
